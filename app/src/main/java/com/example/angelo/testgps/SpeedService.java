...
public class SpeedService extends Service implements LocationListener{
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if(intent.getAction() == this.STOP_SERVICE_ACTION) {
            stopForeground(true);
            stopSelf();
        }
    ...
    private void initNotification() {
        ...
        stopIntent.setAction(this.STOP_SERVICE_ACTION);
        ...
        startForeground(this.NOTIFY_ID, mBuilder.build());
    }
    ...
    private void setSpeed(Float speed) {
        Integer speedInt = Math.round(speed.floatValue());
